...
public class BrandService : IBrandService
{
    ...
    public async Task<Result<BrandDto>> GetBrandDetailsAsync(Guid id)
    {
        var spec = new BaseSpecification<Brand>();
        var brand = await _repository.GetByIdAsync<Brand, BrandDto>(id, spec);
        return await Result<BrandDto>.SuccessAsync(brand);
    }
